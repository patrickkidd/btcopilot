...
def test_create(logged_in):
    response = logged_in.post("/therapist/discussions/")
...
def test_get(logged_in, discussions):
    ...
    db.session.commit()
    response = logged_in.get(f"/therapist/discussions/{discussion.id}")
    assert response.status_code == 200
...
def test_get_404(auditor):
    response = auditor.get("/therapist/discussions/99999")
...
def test_get_statements(auditor, discussion):
    ...
    response = auditor.get(f"/therapist/discussions/{discussion_id}/statements")
    assert response.status_code == 200
    data = response.get_json()
