...
def test_access_right_update(flask_app, test_session, test_user, test_user_2):
    ...
    assert free_diagram.access_rights[0].right == btcopilot.ACCESS_READ_WRITE
    ...
    with flask_app.test_client(user=test_user) as client:
        response = client.patch(
            ...
            data=pickle.dumps(
                {"right": btcopilot.ACCESS_READ_ONLY, "session": test_session.token}
            ),
        )
    test_user = User.query.get(test_user_id)
    test_user_2 = User.query.get(test_user_2_id)
    assert response.status_code == 200
    assert test_user.free_diagram.access_rights[0].right == btcopilot.ACCESS_READ_ONLY
