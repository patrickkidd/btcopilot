    ...
    for ai_person in ai_people:
        ...
        for gt_person in gt_remaining:
            ...
            if ai_person.parents is not None and gt_person.parents is not None:
                resolved_ai_parent = id_map.get(ai_person.parents)
                if resolved_ai_parent is not None:
                    # AI parent ID was resolved — require match
                    if resolved_ai_parent != gt_person.parents:
                        parents_match = False
                # else: AI parent ID not in id_map (likely a PairBond ID that
                # hasn't been matched yet). Skip parents comparison since
                # PairBond matching happens after people matching — we can't
                # resolve PairBond IDs at this stage.
