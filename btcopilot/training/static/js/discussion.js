...
function componentExtractedData(extractedData, cumulativePdp, thumbsDown, submitted, componentId, editableMode = false, messageId = null, editedExtraction = null, feedbackId = null) {
    ...
    return {
        ...
        createPerson() {
            const personId = this.nextNegativeId();
            const personDelta = {
                id: personId,
                name: 'New Person',
                last_name: null,
            ...
            const seen = getUsedIds(this, 'people');
            if (seen.has(personId)) {
                alert(`Warning: ID ${personId} is already in use. This will cause a collision in the cumulative PDP. The ID should have been ${this.nextNegativeId()}.`);
            }

            if (!this.extractedData.people) this.extractedData.people = [];
            this.extractedData.people.push(personDelta);
            this.personDataVersion++;
            this.autoSave();
        },

        editPerson(event) {
            const button = event.target.closest('.edit-person-button');
            if (!button) return;

            document.querySelectorAll('.person-edit-dropdown, .event-edit-dropdown').forEach(d => d.style.display = 'none');
            ...
            if (!dropdown || !dropdown.classList.contains('person-edit-dropdown')) {
                ...
                peopleArray.forEach(person => {
                    ...
                    item.addEventListener('click', () => {
                        this.addPersonUpdate(person.id);
        ...
        addPersonUpdate(personId) {
            if (!personId) return;

            // Look up existing person's name from all sources
            let existingName = null;
            const numericId = typeof personId === 'string' ? parseInt(personId) : personId;

            const diagramPeople = window.diagramPeople || [];
            let existingPerson = diagramPeople.find(p => p.id === numericId);
            if (existingPerson && existingPerson.name) {
                existingName = existingPerson.name;
            }

            if (!existingName && this.cumulativePdp && this.cumulativePdp.people) {
                existingPerson = this.cumulativePdp.people.find(p => p.id === numericId);
                if (existingPerson && existingPerson.name) {
                    existingName = existingPerson.name;
                }
            }

            if (!existingName && this.extractedData && this.extractedData.people) {
                existingPerson = this.extractedData.people.find(p => p.id === numericId);
                if (existingPerson && existingPerson.name) {
                    existingName = existingPerson.name;
                }
            }
        ...
        createEvent() {
            ...
            const seen = getUsedIds(this, 'events');
            if (seen.has(eventId)) {
                alert(`Warning: ID ${eventId} is already in use. This will cause a collision in the cumulative PDP. The ID should have been ${this.nextNegativeId()}.`);
            }
        ...
        editEvent(event) {
            const button = event.target.closest('.edit-event-button');
            if (!button) return;
            ...
            if (!dropdown || !dropdown.classList.contains('event-edit-dropdown')) {
                ...
                diagramEvents.forEach(evt => {
                    if (evt.id) {
                        const item = document.createElement('a');
                        item.className = 'dropdown-item';
                        item.style.cssText = 'display: block; padding: 0.375rem 1rem; cursor: pointer; color: var(--bulma-dropdown-content-background-color, white);';
                        const title = this.formatEventTitle(evt);
                        item.textContent = `${evt.id}: ${title}`;
                        item.addEventListener('mouseover', () => {
                            item.style.backgroundColor = 'var(--bulma-dropdown-content-background-color, white)';
                            item.style.color = 'var(--bulma-text, #4a4a4a)';
                        });
                ...
                if (this.cumulativePdp && this.cumulativePdp.events && this.cumulativePdp.events.length > 0) {
                    this.cumulativePdp.events.forEach(evt => {
                        if (evt.id) {
                            const item = document.createElement('a');
                            item.className = 'dropdown-item';
                            item.style.cssText = 'display: block; padding: 0.375rem 1rem; cursor: pointer; color: var(--bulma-dropdown-content-background-color, white);';
        ...
        addEventUpdate(eventId) {
            if (!eventId) return;

            const sourceEvent = this.cumulativePdp?.events?.find(e => e.id === eventId);
            if (!sourceEvent) {
                alert('Event not found in cumulative data');
                return;
            }

            const eventDelta = JSON.parse(JSON.stringify(sourceEvent));
        ...
        getPersonName(personId, version = null) {
            ...
            // Collect all people from all sources - deltas first, then database
            const allPeople = [];

            // Add from extracted data first (highest priority - current statement deltas)
            ...
            // Add from diagram (database people) LAST - fallback only
            const diagramPeople = window.diagramPeople || [];
        ...
        createEventPerson(eventIndex) {
            ...
            if (!Array.isArray(this.extractedData.people)) {
                this.extractedData.people = [];
            }

            this.extractedData.people.push(personDelta);
            this.extractedData.events[eventIndex].person = personId;
            this.personDataVersion++;
            this.autoSave();
        },

        editEventPerson(event, eventIndex) {
            const button = event.target.closest('.edit-event-person-button');
            if (!button) return;

            document.querySelectorAll('.person-edit-dropdown, .event-edit-dropdown, .event-person-dropdown, .event-spouse-dropdown, .event-child-dropdown').forEach(d => d.style.display = 'none');
            ...
            peopleArray.forEach(person => {
                ...
                item.addEventListener('click', () => {
                    this.setEventPerson(eventIndex, person.id);
                    dropdown.style.display = 'none';
                });
                dropdown.appendChild(item);
            });
        ...
        setEventPerson(eventIndex, personId) {
            this.extractedData.events[eventIndex].person = personId;
            this.autoSave();
        },
        ...
        createEventSpouse(eventIndex) {
            ...
            this.extractedData.people.push(personDelta);
            this.extractedData.events[eventIndex].spouse = personId;
            this.personDataVersion++;
            this.autoSave();
        },

        editEventSpouse(event, eventIndex) {
            const button = event.target.closest('.edit-event-spouse-button');
            if (!button) return;
            ...
            peopleArray.forEach(person => {
                ...
                item.addEventListener('click', () => {
                    this.setEventSpouse(eventIndex, person.id);
                    dropdown.style.display = 'none';
                });
                dropdown.appendChild(item);
            });
        ...
        setEventSpouse(eventIndex, personId) {
            this.extractedData.events[eventIndex].spouse = personId;
            this.autoSave();
        },
        ...
        createEventChild(eventIndex) {
            ...
            this.extractedData.people.push(personDelta);
            this.extractedData.events[eventIndex].child = personId;
            this.personDataVersion++;
            this.autoSave();
        },

        editEventChild(event, eventIndex) {
            const button = event.target.closest('.edit-event-child-button');
            if (!button) return;
            ...
            peopleArray.forEach(person => {
                ...
                item.addEventListener('click', () => {
                    this.setEventChild(eventIndex, person.id);
                    dropdown.style.display = 'none';
                });
                dropdown.appendChild(item);
            });
        ...
        setEventChild(eventIndex, personId) {
            this.extractedData.events[eventIndex].child = personId;
            this.autoSave();
        },
    ...
    return {
        ...
        selectFeedback(index, feedback) {
            ...
            if (index === -1) {
                ...
                const adminFeedback = this.allFeedback.find(f => f.auditor_id === window.discussionConfig.currentAuditor);
...
document.body.addEventListener('sse:message', function(event) {
    try {
        ...
        if (data.type === 'new_message' && data.discussion_id === window.discussionConfig.discussionId) {
                    ...
                    <div class="column">
                        ...
                        <div class="field">
                            <label class="label is-small">${
                                event.kind === 'birth' || event.kind === 'adopted' ? 'Parent 1' :
                                event.kind === 'death' ? 'Person' :
                                event.kind === 'bonded' || event.kind === 'married' || event.kind === 'separated' || event.kind === 'divorced' ? 'Partner 1' :
                                'Person'
                            } (ID)</label>
                            <div class="control">
                        ...
                        <div class="field" style="display: ${['birth', 'adopted'].includes(event.kind) ? 'block' : 'none'}">
...
function saveSpeakerMapping(speakerId, personId) {
    ...
    fetch(`${window.discussionConfig.urls.speakersUpdate}`.replace('/0', `/${speakerId}`), {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
...
function createNewPerson() {
    ...
    fetch(`${window.discussionConfig.urls.speakersUpdate}`.replace('/0', `/${currentNewPersonSpeakerId}`), {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
...
function clearExtractedData(discussionId) {
    ...
    if (window.isAdmin) {
        // Admin: read current value from dropdown
        const auditorSelect = document.getElementById('auditor-filter');
        if (auditorSelect) {
            targetAuditor = auditorSelect.value;
...
document.addEventListener('DOMContentLoaded', function() {
    if (window.location.hash) {
        const statementId = window.location.hash.substring(1);
        const element = document.getElementById(statementId);
        if (element) {
            setTimeout(() => {
                element.scrollIntoView({ behavior: 'smooth', block: 'center' });

                // Get only immediate td children of the row
                const cells = Array.from(element.children).filter(child => child.tagName === 'TD');

                // Set yellow background on all cells first without transition
                for (let i = 0; i < cells.length; i++) {
                    cells[i].style.setProperty('background-color', '#fff9c4', 'important');
                    cells[i].style.setProperty('transition', 'none', 'important');
                }

                // Force reflow to ensure yellow is applied
                element.offsetHeight;

                // Now enable transition and fade to transparent on all cells
                setTimeout(() => {
                    for (let i = 0; i < cells.length; i++) {
                        cells[i].style.setProperty('transition', 'background-color 3s ease', 'important');
                        cells[i].style.setProperty('background-color', 'transparent', 'important');
                    }
                }, 50);
            }, 300);
        }
