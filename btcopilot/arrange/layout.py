...
def _label_px(person):
    ...
    return max(0, int(0.2 * sz) + label_text_w - LABEL_OVERLAP_TOLERANCE)
...
def _is_descendant(by_id, ancestor_id, candidate_id, max_depth=10):
    """True if candidate_id is in ancestor_id's descendant tree."""
    if ancestor_id == candidate_id or max_depth == 0:
        return False
    p = by_id.get(candidate_id, {})
    for par in (p.get("parent_a"), p.get("parent_b")):
        if par == ancestor_id:
            return True
        if par and _is_descendant(by_id, ancestor_id, par, max_depth - 1):
            return True
    return False
...
def _compute_y_levels(by_id):
    ...
    Skip partner-Y equalization when one partner is the other's descendant
    (incest topology) — preserves child-below-parent invariant over partner equality.
    """
    ...
    while changed and iters < 50:
        ...
        for pid, p in by_id.items():
            child_sz = _px(p)
            for par in [p.get("parent_a"), p.get("parent_b")]:
                # Skip self-parent (data corruption: person listed as their own parent)
                if par and par != pid and par in y:
