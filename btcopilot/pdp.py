...
def _normalize_name(name: str | None) -> str | None:
    """Normalize a person name for dedup matching: lowercase, collapse whitespace."""
    if name is None:
        return None
    return " ".join(name.lower().split())


def dedup_against_committed(deltas: PDPDeltas, diagram_data: DiagramData) -> dict:
    """
    Remove extracted items that duplicate already-committed diagram data.

    Three-phase deterministic post-filter:
      Phase 1: People by normalized name
      Phase 2: PairBonds by unordered person dyad (after person remapping)
      Phase 3: Self-describing events by kind + person refs (Shift excluded)

    Returns stats dict with counts of removed duplicates.
    """
    stats = {"people_removed": 0, "pair_bonds_removed": 0, "events_removed": 0}
    ...
    for p in diagram_data.people:
        ...
        if norm and "id" in p:
            committed_name_to_id[norm] = p["id"]

    person_remap: dict[int, int] = {}  # removed PDP ID -> committed ID
    kept_people = []
    for person in deltas.people:
        norm = _normalize_name(person.name)
        if norm and norm in committed_name_to_id and person.id is not None and person.id < 0:
            person_remap[person.id] = committed_name_to_id[norm]
            stats["people_removed"] += 1
        else:
            kept_people.append(person)
    deltas.people = kept_people

    # Apply person remapping to all remaining references
    _remap_person_refs(deltas, person_remap)

    # --- Phase 2: PairBonds by dyad (after person remapping) ---
    ...
    for pb in diagram_data.pair_bonds:
        ...
        if a is not None and b is not None and "id" in pb:
            committed_dyad_to_id[tuple(sorted([a, b]))] = pb["id"]

    pb_remap: dict[int, int] = {}  # removed PDP pair bond ID -> committed ID
    kept_pair_bonds = []
    for pb in deltas.pair_bonds:
        if pb.person_a is not None and pb.person_b is not None and pb.id is not None and pb.id < 0:
            dyad = tuple(sorted([pb.person_a, pb.person_b]))
            if dyad in committed_dyad_to_id:
                pb_remap[pb.id] = committed_dyad_to_id[dyad]
                stats["pair_bonds_removed"] += 1
                continue
        kept_pair_bonds.append(pb)
    deltas.pair_bonds = kept_pair_bonds

    # Apply pair bond remapping to person.parents
    for person in deltas.people:
        if person.parents is not None and person.parents in pb_remap:
            person.parents = pb_remap[person.parents]

    # --- Phase 3: Self-describing events by kind + person refs ---
    # Build lookup sets from committed events
    committed_event_keys: set[tuple] = set()  # (kind_value, person_id)
    committed_offspring_keys: set[tuple] = set()  # (kind_value + "_child", child_id)
    for e in diagram_data.events:
        kind_val = e.get("kind")
        if kind_val is None:
            continue
        try:
            kind = EventKind(kind_val)
        except ValueError:
            continue
        if not kind.isSelfDescribing():
            continue
        person_id = e.get("person")
        if person_id is not None:
            committed_event_keys.add((kind_val, person_id))
        child_id = e.get("child")
        if child_id is not None and kind.isOffspring():
            committed_offspring_keys.add((kind_val + "_child", child_id))
    ...
    for event in deltas.events:
        if event.id >= 0:
            ...
            continue
        if not event.kind.isSelfDescribing():
            # Shift events are intentionally kept — require description matching
            kept_events.append(event)
            continue
        matched = False
        if event.person is not None:
            if (event.kind.value, event.person) in committed_event_keys:
                matched = True
        if not matched and event.child is not None and event.kind.isOffspring():
            if (event.kind.value + "_child", event.child) in committed_offspring_keys:
                matched = True
        if matched:
    ...
    total = stats["people_removed"] + stats["pair_bonds_removed"] + stats["events_removed"]
    if total > 0:
        _log.info(
            f"dedup_against_committed: removed {stats['people_removed']} people, "
            f"{stats['pair_bonds_removed']} pair bonds, {stats['events_removed']} events"
        )

    return stats


def _remap_person_refs(deltas: PDPDeltas, person_remap: dict[int, int]) -> None:
    """Apply person ID remapping to all references in deltas."""
    if not person_remap:
        return
