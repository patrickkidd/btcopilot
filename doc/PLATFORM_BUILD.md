...
**Observability (R-0370, supersedes the Datadog half of R-0328)**: Grafana Cloud Free, stack alaskafamilysystems. Alloy on the box (`deploy/chat/alloy/config.alloy`) ships host and container metrics, every container's log lines, and OTLP traces; the browser sends errors and web vitals to Frontend Observability app fd-app through the Faro SDK (`web/src/telemetry.ts`); each coach model call is a `coach.turn` span with token counts and no message text. Session Replay is a Cloud preview requested by form. The token is `GRAFANA_CLOUD_TOKEN` in the box's secrets file. DATADOG.md is the record of the rejected option. Dashboards: fd-chat (people and cost), fd-box (health), fd-features (what people use, from product_events); Grafana reads Postgres through the PDC agent fd-pdc as role grafana.
