...
**On the line numbers.** Every build value here was read from the source at the commit this file was last regenerated against, and the front end has been under active edit since. Line citations therefore drift, and they drift silently: a citation that now points at a real but unrelated line still looks fine. A checker lives at `/Users/patrick/.claude/jobs/1674c7c2/tmp/citecheck.py`, run with the worktree path as its argument; it reports how many of the file's citations still land on code. At the last run, 27 of 209 were stale, three of them past the end of `caption.ts`, concentrated in the files the symbol work is touching. Treat a line number as a hint about where to look, never as evidence: re-read the symbol before acting on a row. When repairing this file, cite the function, class or selector name instead, because a name survives an edit and a line number does not.
...
| Emotional field, rings | three `<circle cx cy r=24 fill=none class="A d" stroke-width="2.4" opacity="0">`, `r` animating 18 to 170 over 1.65s, begins staggered at 0s / .55s… | three rings, `r` 18 to 170 over 1.65s, begins 0s / .55s / 1.1s, opacity `.75;.45;0`, stroke-width 2.4 (moves.ts `rings`) [6aa6717] | MET |
...
| Conflict, sparks | one zigzag polyline `points="70,55 82,48 92,62 102,49 112,61 122,49 132,61 144,55"`, `.A d`, stroke-width 2.4; plus an 8-line radial spark cluster… | one zigzag polyline at stroke-width 2.4 plus the radial burst at its middle at stroke-width 2 on `sparkp .5s` (moves.ts `sparks`) [6aa6717] | MET |
...
| Away | mirror of toward: mover `cx=120 cy=60 r=14`, `awaywalk` translateX 0 to -68px between 50% and 94% over 8s; trailing line `.A d` stroke-width 2.4 `d… | the mirror of toward with `flow` reversed and the arrow leading the way out, its length cut to the room the board leaves (moves.ts `arrow`) [6aa6717] | MET |
...
| Projection, spike static | 8 lines `.A d` stroke-width 1.8, each with its own opacity flicker between 0.27s and 0.58s; the group scaling `1;1;.12;.12` on the parent and `.12;… | 8 spikes at stroke-width 1.8, each with its own flicker between .27s and .58s, the group scaling down on the parent as it scales up on the child (moves.ts `spikes`) [6aa6717] | MET |
| Projection, flow arrow | `x1=68 y1=57 x2=156 y2=83` `.A d` stroke-width 2.8 `stroke-dasharray="9 7"`, `flow .45s linear infinite`; head `<polygon points="165,86 150,76 153,… | stroke-width 2.8, dasharray `9 7`, `flow .45s` (theme.css `.mv-flow`) [6aa6717] | MET |
| Anxiety shake primitive | `pshakeC` / `cshakeC`, identical: `0%,100%(0,0) 20%(-2.5,1.5) 45%(2.5,-1.5) 70%(-2,1) 85%(2,-1)`, 1.1s infinite | `@keyframes pshakeC` verbatim, 1.1s infinite (theme.css) [6aa6717] | MET |
| Anxiety variable | the projection rig on a single figure: sharp base `cx=115 cy=64 r=17` `.A` stroke-width 2.2 always present; ghost-double with its own blur filter a… | the projection rig on one figure and no field rings (moves.ts `draw`) [6aa6717] | MET |
...
| Triangle, outside | two static corners: circle `cx=88 cy=40 r=13`, rect `x=128 y=27 w=26 h=26`; two tension zigzags `points="104,84 100,76 106,68 98,60 103,52"` `.A d`… | two tension zigzags at stroke-width 2 on `sparkp`, gated by `tenspre`, and the mover walks out on `tremout` (moves.ts `draw`) [6aa6717] | MET |
| Fusion | person A circle `cy=64 r=15` with `cx` `70;70;101;101`; person B rect `y=50 w=28 h=28` with `x` `140;140;112;112`, both on keyTimes `0;.2;.55;1` ov… | three straight bands at -6 / 0 / +6, stroke-width 2.6, holding from the first frame and shrinking as the two are drawn in, then the shared field ring (moves.ts `bands`) [6aa6717] | MET |
...
| Defined self, the storm | other party rect `x=172 y=50 w=28 h=28` with `btrem2 12s infinite`; two long storm rings `r` 18 to 170 over 1.1s (second `begin=".55s"`), opacity `… | the other party on `btrem2` under the long storm rings and the calming rings, the actor kicked on `hitsync` (moves.ts `storm`) [6aa6717] | MET |
...
| Symptom cross | `translate(146,60)` with two `<rect class="df" rx="1">`, `x=-8 y=-3 w=16 h=6` and `x=-3 y=-8 w=6 h=16`, filled solid green | two solid green rects, 16x6 and 6x16, `rx=1` (moves.ts `cross`) [6aa6717] | MET |
| Symptom up arrow (worse) | shaft `x1=172 y1=74 x2=172 y2=48` `.A d` stroke-width 2.6, head `<polygon points="172,42 164,52 180,52" class="df">`; opacity `0;0;1;1;0;0` on keyT… | a shaft at stroke-width 2.6 with a filled polygon head, visible 14-48% of 8s (theme.css `symworse`) [6aa6717] | MET |
| Symptom down arrow (better) | shaft `x1=172 y1=48 x2=172 y2=74` `.A d` stroke-width 2.6, head `<polygon points="172,80 164,70 180,70" class="df">`; opacity keyTimes `0;.5;.56;.9… | the same drawing pointing down, visible 56-90% of the same 8s (theme.css `symbetter`) [6aa6717] | MET |
| Functioning | one circle `cx=115 cy=64 r=17 fill=none stroke-width="2.6"`; `stroke-dasharray` animating `106 0; 106 0; 5 6; 5 6; 106 0; 106 0` on keyTimes `0;.12… | one circle at stroke-width 2.6 whose `stroke-dasharray` animates on the ratified key times, with `fcolor` losing the green for ink (moves.ts `functioning`) [6aa6717] | MET |
...
| History marks | earlier moves stay on the board at `opacity: 0.16`, behind the current gesture; they fade in over `.32s ease` from 0 | earlier moves at opacity .16 behind the current gesture, fading in over .32s ease (board.ts `history`) [1c5488c] | MET |
...
| Current-mover emphasis | mover label fill `--move`, weight 600; every other label fill `--faint`, weight 400 | the mover's name in `--move` at 600, every other name `--faint` at 400 (theme.css `.nm.on`) [6aa6717] | MET |
...
| Board step controls | `◀` plain `.btn` and `▶ next move` `.btn.primary`, min-height 44px, padding `0 14px`, border-radius 4px | `◀` plain and `▶ next move` primary, min-height 44px, padding `0 14px`, border-radius 4px, each disabled at its end (picture.ts `renderBoard`) [1c5488c] | MET |
...
| element | spec value | build value | status |
|---|---|---|---|
| `triangle` — a triangle over three people | no mockup fixes the geometry | three people at full height with the ratified tension zigzag on all three sides. ASSUMED: no mockup fixes this geometry (picture.ts `stage`) [d532d67] | MET |
...
| Slide gesture | SVG `animateTransform` translate, `dur="1.2s" fill="freeze"` | `<animateTransform type="translate">` on each figure that walks, on its move's own key times with an ease-in-out spline (moves.ts `figure`) [6aa6717] | MET |
