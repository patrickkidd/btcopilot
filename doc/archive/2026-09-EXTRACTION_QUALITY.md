...
| Unresolvable person ref → orphaned committed structure / scene crash | `validate_pdp_deltas` event person/spouse/child positive-not-committed branch (added 2026-05-17, FD-319) | `fix_unresolved_person_refs` (drops events/pair_bonds with positive ref not committed or negative ref absent from delta/PDP) + `commit_pdp_items` orphan-bond skip + `_create_inferred_pair_bond_items` resolve guard | `fix_unresolved_person_refs:` , `Skipping orphaned pair bond` | Found via real diagram 1928: LLM emitted a `bonded` event with positive person ids (821/819) that were never committed; only pair_bond endpoints were validated, not event refs, so it reached commit and `_create_inferred_pair_bond_items` fabricated an orphaned bond → scene `Marriage.personA()` None → accept crashed, diagram corrupted. Regression tests `test_commit_skips_orphaned_pair_bonds` + (pending) validator/repair unit tests. Not yet deployed. | Watching |
