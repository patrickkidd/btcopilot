> **Archived 2026-09-23.** Superseded by the coach editing the record turn by turn ([PROMPT_ENGINEERING_LOG](../PROMPT_ENGINEERING_LOG.md), R-0414). Kept for historical reference.
...
**Key insight**: thinking_budget=1024 is the single biggest lever (+43% Events F1 on 2.5-flash, +139% on flash-lite). Flash-lite matches 2.5-flash quality at ~6x lower cost. See report: `fdserver/training/induction-reports/2026-03-04_13-15-00--model-evaluation-flash-lite/`
...
`fdserver/training/induction-reports/2026-05-16_08-40-13--fd319-prompt-idempotency/`.
...
19. infer_parents_from_birth_events deterministic repair (FD-324, 2026-05-20): after extraction, infer Person.parents from birth events that have person+spouse+child set. Find the matching PairBond by dyad and set Person.parents on the child if currently null. No prompt change. Baseline ParentChild F1 0.366 → 0.782 (+114%); average LCC 51% → 89.5% (+38.5 pp, target ≥80% met); Aggregate F1 0.655 → 0.651 (within noise). Root cause: LLM follows people-first ID assignment order and cannot forward-reference pair bond IDs when creating Person objects — repairs the silent omission without needing the LLM to do anything different. Pairs with fix_committed_person_duplicates and fix_birth_event_self_references as the third deterministic repair in the pipeline.
20. Gate-and-dock directed connectivity repair (FD-338, 2026-06-09): deterministic connected-components gate (free) + ONE targeted full-transcript repair call (DOCK_PROMPT, pass-3 model, no cursor rule, explicit cross-turn pronoun-resolution instruction, edges-only output, verbatim-quote requirement) + programmatic edge applier, accepted only if floating-component count strictly drops. Probe: pronoun-bridge couple docked 4/4 with verified-correct antecedent, zero false attaches of legit-disconnect singletons, post-dock LCC 93.3-97.0% (pre 72-90%); production battery 7/7 strict-drop accepts, dock kill-gate clean, F1 within ±0.05. Replaced K=4 union consensus as the rebuild core (DEFAULT_K=1; ~$0.11 vs $0.23/rebuild). NOT a contradiction of failed-#18 (global completion pass): the difference is deterministic gap DETECTION + named-target directed search + quote grounding + accumulation scenario. Precision limit: quote gate verifies evidence existence, not relation type (1/13 distinct edges right-family/wrong-generation); dock edges ride staged review, never auto-commit. Do not generalize to untargeted repair passes — #18 in "things that failed" still stands.
21. gemini-3.6-flash for extraction (2026-07-22): cold baseline Events 0.517 vs same-day prod 0.389 (+33%), Aggregate 0.695 vs 0.648, SARF macro 0.417 vs 0.332 — with the P3 reviewer unchanged. Best Gemini-family Events on record at flash-tier cost. Latency ~76s/disc (2.4x prod). See fdserver/training/induction-reports/2026-07-22_07-56-26--gemini-3.6-flash/.
22. Saturation example in PASS2 (2026-07-22, gemini-3.6-flash): the fable-5 saturation example (one pattern event + texture in notes; distinct symptoms stay separate) transfers to 3.6-flash — Events +0.030 (3-run mean), gain arrived via recall (more distinct symptom events), not the intended texture-FP cut.
23. Couple-slot symmetric event matching in f1_metrics.py (2026-07-22): married/divorced/separated/bonded match on unordered {person, spouse}. Fixes the long-documented married slot asymmetry for ALL models; prod Events +0.051 from the metric fix alone. Together with #25 defines benchmark era E4 — E3/E4 numbers not comparable.
25. Year-precision date rule in f1_metrics.py (2026-07-22, Patrick's call): a Jan-1 date marked certain is a year-only fact — matches any date in that calendar year. Removes the ±7-day gate that made correct-year AI dates unmatchable against year-anchored GT dates (e.g. disc 37 Helen death). Part of era E4.
24. gemini-3.6-flash self-review on Pass 3 (2026-07-22): 3.6-flash extraction + 3.6-flash SARF review beats 3.6-flash + 3-flash review on everything. Final-ruler 3-run means: Events 0.544 vs prod 0.413, Agg 0.704 vs 0.652. Recommended production config (E4 ruler). SARF macro varies heavily run-to-run (0.451/0.392 batch means, identical config) — rank configs by Events/Agg.
...
29. Current-gen non-Gemini models (2026-03-04) — Tested gpt-5.2, gpt-5-mini, gpt-5-nano, o4-mini, gpt-4.1 (OpenAI) and grok-4-fast-reasoning, grok-4-1-fast-reasoning (xAI). Results: gpt-5-mini had highest Events F1 (0.410) but 460s latency disqualifies. gpt-5.2 ties gemini-3-flash on Events (0.397) but Bonds -37% and 196s. o4-mini baseline-tier quality at 289s. All non-Gemini models have weak Pair Bond extraction (0.444-0.825 vs baseline 0.819). Latency 196-831s vs gemini-3-flash 74s. No non-Gemini model justifies production deployment or prompt engineering investment. See full report: `fdserver/training/induction-reports/2026-03-04_15-36-39--model-evaluation-frontier/`
30. Birth-dedup prompt rule on gemini-3.6-flash (2026-07-22) — the exact text kept-neutral on fable-5 INCREASED birth FPs 19->28 on 3.6-flash (added birth salience = more births). Prompt-rule transfer across models is not safe even within a provider.
31. Bounded birth completeness check (2026-07-22) — "exactly one birth per stated age, do not infer, remove extras" cut birth FPs only 21->18 while ParentChild fell 0.836->0.797: birth events feed infer_parents_from_birth_events, so birth precision and ParentChild recall are COUPLED. Prompt-side birth suppression is net negative.
32. Dyadic present-tense pattern rule (2026-07-22) — narrowing the Do-NOT list + "patterns between two specific people ARE shift events (statement-date anchored)" opened the gate: shift FPs 21->29, TP down. The Do-NOT list is load-bearing; GT codes only a subset of dyadic patterns and the model cannot tell which. Third confirmation that encouraging extraction fails (see #9, #12).
33. Reviewer-only upgrade over flash-lite extraction (2026-07-22) — flash-lite P1+P2 + gemini-3.6-flash P3 loses to prod on SARF (0.349 vs 0.367). The fable-5 hybrid finding does NOT generalize: a better reviewer only helps over sufficiently good extraction. SARF lift requires upgrading the extraction pass first.
...
- All API costs are negligible: $0.004-0.065 per extraction across all tested models. A "premium AI" tier cannot be justified by cost differences — the delta between cheapest and most expensive model is $0.061. Quality (not cost) should drive model selection. (2026-03-04, frontier model eval) **EXCEPTION (2026-06-09): claude-fable-5 costs ~$0.81/discussion (12-250x all prior models) — the first model where cost IS a deployment variable.**
- claude-fable-5 cold baseline (2026-06-09, Pass 1+2 only, Pass 3 unchanged gemini-3-flash, 3-run mean): Aggregate 0.721 vs same-day prod 0.658; Events 0.592 vs 0.427 (+39%, highest ever on this benchmark); People/ParentChild ~equal; PairBonds 0.785 vs 0.824 (mild dip — the catastrophic non-Gemini bond collapse did NOT occur). Run variance 3-5x lower than Gemini (agg spread 0.715-0.729). Latency 71-179s/discussion (async-only). Anthropic constrained decoding rejects the PDPDeltas schema (union-param limit + grammar timeout) — adapter uses schema-in-prompt + client-side validation, 0 parse failures in ~100 calls. (2026-06-09, fable-5-extraction worktree)
- **Hybrid per-pass model selection WORKS with claude-fable-5 on Pass 3** (2026-06-09): flash-lite P1+P2 + fable-5 SARF review = SARF macro 0.535 vs 0.375 prod (+43%, 2-run mean) with Events/Aggregate unchanged, at ~$0.20/discussion. This overturns the 2026-03-04 "hybrid per-pass selection doesn't help" finding (#19 in failed) — that tested Gemini-tier swaps where thinking budget was the bottleneck; a genuinely stronger reviewer model DOES lift SARF. All-Fable (P1+P2+P3) = records everywhere: Agg 0.731, Events 0.617, SARF macro 0.621 (F=0.770 — historical weakest variable), ~$1.30/disc. Events gain comes only from Fable extraction; SARF gain only from Fable review — levers are independent. (2026-06-09)
- Prompt induction on claude-fable-5 CONVERGED AT COLD BASELINE (4 iterations: 3 kept within noise, 1 reverted). Gemini-tuned prompts transfer unchanged; the +39% Events gain is model-native, not prompt-extractable. Future frontier-model evals: cold baseline + 1-2 targeted iterations, not full 10-iteration induction. Fable 5 decoding is near-deterministic per discussion (byte-identical re-runs observed) — single-run iteration tests are more trustworthy than on Gemini. (2026-06-09)
- dateCertainty="unknown" for inferred dates (2026-06-09, fable-5) — REVERTED: Events −0.016; opening the F1 date gate raises FPs elsewhere and risks false TPs. Don't retry without a tighter mechanism.
- match_events married-event person/spouse slot asymmetry suppresses measured F1 for ALL models: AI picking the other partner as `person` can never match GT. Metric fix (canonicalize spouse pairs symmetrically in f1_metrics.py), not a prompt problem. (2026-06-09)
...
- GT year-anchored dates coded dateCertainty=certain used to force the +/-7-day gate on year-only facts. RESOLVED in the E4 scorer (year-precision rule, worked #25) rather than by re-coding GT. (2026-07-22)
- gemini-3.6-flash run variance on Events is Gemini-typical (single-run spread up to +/-0.04); 3-run means remain mandatory, unlike claude-fable-5. (2026-07-22)
...
Constraints (still apply if revived): follow `induction_agent.md` protocol, log to `fdserver/training/induction-reports/`, multi-run averaging (3+ runs) required for signal, production model `gemini-3-flash-preview` thinking=1024.
...
| [PROMPT_OPTIMIZATION.md](2026-09-PROMPT_OPTIMIZATION.md) | Prompt optimization process |
| [F1_METRICS.md](2026-09-F1_METRICS.md) | F1 calculation details |
