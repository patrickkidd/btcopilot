...
| Coach tool: show on the picture (**ruled**) | New record slice, `views` | Picks one of a fixed set of views (triangle, span, two moments compared, sequence of moves) and passes ids and dates from the record; anything that does not resolve to stored data errors instead of drawing [R-0075]. | Yes | Pro ignores an unknown key today (see section 4). |
