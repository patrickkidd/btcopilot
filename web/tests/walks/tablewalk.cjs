...
//   node tablewalk.js <admin-invite-url> <width> <height> <tag>
...
(async () => {
  ...
  page.on("requestfailed", (r) => failed.push(`NET ${r.url()} ${r.failure()?.errorText}`));
  ...
  const gates = async (step) => {
    const g = await page.evaluate(() => {
      ...
      for (const e of document.querySelectorAll("body *")) {
        ...
        // A row held open by the swipe gesture is translated left on purpose,
        // which is the ratified behaviour for rename and delete alike.
        if (e.closest(".row.swiped")) continue;
        if (r.right > innerWidth + 1 || r.left < -1)
          out.push(`${e.tagName.toLowerCase()}#${e.id}.${[...e.classList].join(".")} ${Math.round(r.left)}..${Math.round(r.right)}`);
      }
      return { hs: se.scrollWidth > se.clientWidth + 1, out: out.slice(0, 6) };
    });
    check(!g.hs, `${step}: no horizontal scroll`);
    check(g.out.length === 0, `${step}: no box past the viewport edge ${g.out.join(" | ")}`);
  };
