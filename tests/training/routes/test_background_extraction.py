...
from fdserver.therapist.routes.discussions import extract_next_statement
from fdserver.therapist.database import (
    Database,
    PDPDeltas,
    Event,
    Person,
...
@patch("fdserver.therapist.routes.discussions.pdp.update")
...
def test_extract_next_statement_no_statements(flask_app, test_user):
    ...
    with patch("fdserver.therapist.routes.discussions.pdp.update") as mock_pdp_update:
...
def test_extract_next_statement_ordering(mock_pdp_update, flask_app, test_user):
    ...
    discussion2 = Discussion(user_id=test_user.id, summary="Second discussion", extracting=True)
    db.session.add(discussion2)
    db.session.flush()

    discussion1 = Discussion(user_id=test_user.id, summary="First discussion", extracting=True)
