...
# btcopilot standalone - no fdserver dependencies needed for stream tests
...
def test_requires_auditor_or_admin(subscriber, endpoint, method, caplog):
    ...
    with patch("fdserver.therapist.utils.get_auditor_id", return_value="test_auditor"):
...
def test_sse_stream_connection(auditor):
    ...
    with patch("fdserver.therapist.sse.sse_manager.subscribe"):
...
def test_sse_test_endpoint(auditor):
    ...
    with patch("fdserver.therapist.sse.sse_manager.publish"):
