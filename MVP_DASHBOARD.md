...
| ~~T7-11~~ | ~~Fix extraction dedup against committed items~~ | CC+H | ~2 hr | DONE 2026-03-03. Added `dedup_against_committed()` in `pdp.py` — deterministic post-LLM filter that removes people (by name), pair bonds (by dyad), and self-describing events (by kind+person) that match committed diagram items. 34 unit tests in `test_dedup_committed.py`. |
